...
object MainOld {
  ...
  val dimProjectParquetPath = "/glodon/apps/public/dim_project_from_action"

  def projectCompute(baseDataDf: DataFrame) = {
    //allProject 基础数据
    sqlContext.read.load(projectParquetPath).select("pId","gid","first_open_datetime", "last_close_datetime","total_duration","user_id","user_name","email","mobile").registerTempTable("allProject")
  ...
  def projectToHdfs(projectDf : DataFrame)={
    ...
    projectDf.where("is_insert=1").
      ...
      coalesce(30).write.mode("append").save(projectParquetPath)
    //增量存储edw service
  ...
  def insertToEdw(projectDf:DataFrame)={
    projectDf.where("is_insert=1").foreachPartition((iterator: Iterator[Row]) =>{
      ...
      val sql =
        """insert ignore into fact_project_by_product_lock_a(
           id,
           product_id,
           lock_number,
           yun_account_id,
  ...
  def updateProject(projectDf : DataFrame) = {
    ...
    projectDf.where("is_insert=0").foreachPartition((iterator: Iterator[Row]) => {
      lazy val conn: Connection = DriverManager.getConnection(Constants.EDW_JDBC_URL,"webuser","123.c0m")
      conn.setAutoCommit(false)
      var sql = "update fact_project_by_product_lock_a set yun_account_id =if(yun_account_id='N/A',?, yun_account_id),  total_long =total_long + ?, first_open_datetime=?,last_close_datetime =  ?, last_long =  ? where id = ? "
      lazy val ps: PreparedStatement = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY)
      try {
        var count: Long = 0
        iterator.foreach(row => {
  ...
  def dimProjectCompute = {
    ...
    dimProjectDf.foreachPartition((iterator: Iterator[Row]) => {
      lazy val conn: Connection = DriverManager.getConnection(Constants.EDW_JDBC_URL, "webuser", "123.c0m")
      conn.setAutoCommit(false)
      val sql = "insert ignore into dim_project_from_action(pid, project_id, project_name,prjfullpath,prjcost,prjsize,major,duration,utype,regionrule) values (?,?,?,?,?,?,?,?,?,?)"
      lazy val ps: PreparedStatement = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY)
      try {
        var count: Long = 0
        iterator.foreach(row => {
  ...
  def projectSum ={
    ...
    try {
      lazy val ps : Statement = conn.createStatement()
      var sql = "TRUNCATE mdw.fact_project_sum_by_product_lock_a"
      ps.addBatch(sql)
//      sql = "INSERT INTO fact_project_sum_by_product_lock_a(product_id,ver,lock_number,project_cnt) " +
...
//        "GROUP BY product_id , version,lock_number"
      sql = "INSERT INTO fact_project_sum_by_product_lock_a(product_id,ver,lock_number,project_cnt) select product_id ,version, lock_number, count(1) from fact_project_by_product_lock_a GROUP BY product_id , version,lock_number"
      ps.addBatch(sql)
      ps.executeBatch()
      conn.commit()
    } catch {
...
//      conn.setAutoCommit(false)
//      val sql = "insert into batch.fact_project_sum_by_product_lock_b(product_id,ver,lock_number,project_cnt) values (?,?,?,?)"
//      lazy val ps: PreparedStatement = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_READ_ONLY)
//      try {
//        var count: Long = 0
//        iterator.foreach(row => {
